...
class TestSupervisor:
    ...
    async def test_clean_exit_stops_supervisor(self) -> None:
        from memoriant_ops_bot.run import supervisor
        ...
        with (
            ...
            patch("memoriant_ops_bot.run.WATCH_DIR", Path("/nonexistent")),
        ):
            # supervisor() should exit when child returns 0
            await supervisor()

    async def test_restart_exit_code_respawns(self) -> None:
        from memoriant_ops_bot.run import EXIT_RESTART, supervisor
    ...
    async def test_fast_crash_escalates_backoff(self) -> None:
        ...
        with (
            ...
            patch("memoriant_ops_bot.run.time") as mock_time,
            patch("memoriant_ops_bot.run.WATCH_DIR", Path("/nonexistent")),
        ):
...
class TestTerminateChild:
    ...
    async def test_terminate_sends_sigterm(self) -> None:
        from memoriant_ops_bot.run import _terminate_child
    ...
    async def test_terminate_kills_on_timeout(self) -> None:
        ...
        with patch("memoriant_ops_bot.run.SIGTERM_TIMEOUT", 0.01):
